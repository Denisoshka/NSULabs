...
const int K_max_pattern_len = 16, K_max_chunk_len = 1024, K_hash_base = 3;
...
int get_hash(our_custom_array* blank, const our_custom_array* pattern) {
  int degree = 1;
  for (int index = 0; index < pattern->array_len;
       index++, degree *= K_hash_base) {
    blank->hash += (blank->array[index] % K_hash_base) * degree;
  }
  return degree / 3;
}

void text_rewrite(our_custom_array* text, const our_custom_array* pattern,
                  FILE* stream_in) {
  int where_rewrite = text->additional - pattern->array_len + 1;
  const int iterations_quantity
          = pattern->array_len - text->additional + text->array_len - 1;

  for (int iteration = 0; iteration < iterations_quantity;
       iteration++, where_rewrite++) {
    text->array[iteration] = text->array[where_rewrite];
  }

  text->array_len
          = (int) fread(text->array + iterations_quantity,
                        sizeof(unsigned char),
                        text->array_len - iterations_quantity, stream_in)
          + iterations_quantity;
  text->additional = pattern->array_len - 1;
}

void update_hash_and_get(our_custom_array* text,
                         const our_custom_array* pattern, const int flag) {
  if (flag) {
    text->hash
            -= text->array[text->additional - pattern->array_len] % K_hash_base;
  }
  else {
    text->hash -= text->array[0] % K_hash_base;
  }
  text->hash /= K_hash_base;
  text->hash += (text->array[text->additional] % K_hash_base)
          * pattern->additional;
}

void search_substring(our_custom_array* text, const our_custom_array* pattern,
                      const int* total_index, FILE* stream_out) {
  const int print_index = *total_index - pattern->array_len + 1;

  for (int pattern_work_index = 0,
           text_work_index = text->additional - pattern->array_len + 1;
       pattern_work_index < pattern->array_len;
       pattern_work_index++, text_work_index++) {
...
int rabin_carp_algorithm(const our_custom_array* pattern, FILE* stream_in) {
  ...
  text.additional = pattern->array_len - 1;
  text.array_len = (int) fread(text.array, sizeof(unsigned char),
                               K_max_chunk_len, stream_in);
