...
int GetNumberFromExpression(const ExpressionArray *Expression, int *Index) {
  int result = 0;
  for (; (*Index < Expression->ArrayLen && isdigit(Expression->Array[*Index]));
       (*Index)++) {
    result *= 10;
...
int Calculate(IntStack *NumberStack, const char Operation) {
  if (IsEmptyIntStack(NumberStack)) {
    return kSyntaxError;
  }
...
int CalculateExpression(const ExpressionArray *Expression,
                        IntStack *NumbersStack, CharStack *OperationsStack) {
  ...
  for (int Index = 0; Index < Expression->ArrayLen; Index++) {
    if (isdigit(Expression->Array[Index])) {
      ...
      for (int CalculationError; Operation != '(';) {
        if ((CalculationError = Calculate(NumbersStack, Operation))) {
      ...
      if (!IsEmptyCharStack(OperationsStack)) {
        for (int CalculationError; !IsEmptyCharStack(OperationsStack)
             && GetPriority(GetCharStack(OperationsStack))
                     >= GetPriority(Expression->Array[Index]);) {
          if ((CalculationError
               = Calculate(NumbersStack, PopCharStack(OperationsStack)))) {
            return CalculationError;
          }
        }
        if (IsFullCharStack(OperationsStack)) {
          fprintf(stderr, "__LINE__ %d", __LINE__);
          return kOtherError;
        }
        PushCharStack(OperationsStack, Expression->Array[Index]);
      }
...
int PrintExpressionResult(IntStack *NumberStack, CharStack *OperationStack,
                          const int Flag) {
  ...
  if (Flag || !IsEmptyCharStack(OperationStack)
      || GetIntStackSize(NumberStack) != 1) {
    if (Flag == kDivisionByZero) {
      fprintf(StreamOut, "division by zero\n");
    }
    if (Flag != kOtherError) {
      fprintf(StreamOut, "syntax error\n");
    }
  }
  else {
...
int main(void) {
  ...
  const int CalculationError = (Expression.ArrayLen == 0)
          ? kSyntaxError
          : CalculateExpression(&Expression, &NumbersStack, &OperationsStack);
