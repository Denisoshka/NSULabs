...
const int K_max_pattern_len = 16, K_max_chunk_len = 1024,
...
void scan_pattern(our_custom_array* pattern, FILE* stream_in) {
  ...
  for (; (1 == fread(&symbol, sizeof(unsigned char), 1, stream_in))
       && symbol != '\n';
       index++) {
...
void text_rewrite(our_custom_array* text, const our_custom_array* pattern,
  ...
  const int K_where_start_rewrite = text->sub_index - pattern->array_len + 1,
            K_iterations_quantity
          = pattern->array_len - text->sub_index + text->array_len - 1;
  ...
  text->array_len
          = (int) fread(text->array + K_iterations_quantity,
                        sizeof(unsigned char),
                        text->array_len - K_iterations_quantity, stream_in)
          + K_iterations_quantity;
  text->sub_index = pattern->array_len - 1;
}

void search_substring(our_custom_array* text, const our_custom_array* pattern,
                      int* total_index, const int* shift_table,
                      FILE* stream_out) {
  for (int pattern_work_index = pattern->array_len - 1,
           text_work_index = text->sub_index;
       pattern_work_index >= 0; pattern_work_index--, text_work_index--) {
    fprintf(stream_out, "%d ",
            *total_index + pattern_work_index - pattern->array_len + 1);
...
void booyer_moore_algorithm(const our_custom_array* pattern,
                            const int* shift_table, FILE* stream_in) {
